...
[Collection("FullApp")]
public class GetBggGameDetailTests
{
    private readonly GameSyncAppFactory _factory;
    private readonly HttpClient _client;
    public GetBggGameDetailTests(GameSyncAppFactory factory)
    {
        _factory = factory;
    ...
    public async Task Retrieving_details_of_existing_game_should_return_it()
    {
        ...
        var (response, result) = await _client.GETAsync<GetBggGameDetail.Endpoint, RequestToIdentifiableObject, GameDetail>(detailGameRequest);

        response.EnsureSuccessStatusCode();

        var expected = new GameDetail
        {
