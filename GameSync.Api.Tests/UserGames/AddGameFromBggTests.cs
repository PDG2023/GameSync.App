...
public class AddGameFromBggTests : TestsWithLoggedUser
{
    ...
    public async Task Adding_an_existing_game_should_return_it_and_persist_it()
    {
        ...
        // act
        var (response, initialGame) = await Client.POSTAsync<AddBggGame.Endpoint, RequestToIdentifiableObject, Game>(gameRequest);
        var (getResponse, retrievedGame) = await Client.GETAsync<GetGame.Endpoint, RequestToIdentifiableObject, Game>(new RequestToIdentifiableObject { Id = initialGame.Id});
